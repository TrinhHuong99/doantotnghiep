<template>
    <b-card>
        <b-modal ref="modal-primary" modal-class="modal-primary" centered hide-footer size="lg" title="Thêm môn thi mới">
            <b-card-text>
                <b-row>
                    <b-col class="mb-1">
        ...
        <b-modal ref="modal-edit-class" modal-class="modal-primary" centered hide-footer size="lg" title="Sửa thông tin môn thi">
            <b-card-text>
                <b-row>
                    <b-col class="mb-1">
                        <b-form-group label="Tên part" label-for="exam-name">
                            <b-form-input v-model="examModalData.name" :state="classNameValidation" id="exam-name" placeholder="Tên môn thi"></b-form-input>
                            <b-form-invalid-feedback v-if="!classNameValidation" id="input-live-feedback">
                                Hãy nhập tên môn thi
                            </b-form-invalid-feedback>
                        </b-form-group>
        ...
        <b-card-text>
            <div class="custom-search d-flex justify-content-between mb-2">
                <b-button variant="success" @click="openModalAdd()">Thêm môn thi mới</b-button>
            </div>
            <b-row>
                <b-col cols="12">
...
export default {
    ...
    created() {
        const self = this
        this.$http.get("/get-subject").then((res) => {
            if (res.data.code === 1) {
